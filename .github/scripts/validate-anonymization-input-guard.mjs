...
if (contract.schemaVersion !== 5 || contract.gate !== 'P7_ANONYMIZATION_INPUT_GUARD') fail('gate/schema mismatch')
if (contract.state !== 'IMPLEMENTED_WITH_LONG_PDF_AND_DOCX_MEDIA_VISUAL_RESIDUAL') fail('state must preserve long-PDF and DOCX-media residual')
...
if (binary?.state !== 'SATISFIED_WITH_BOUNDED_LOCAL_VISUAL_DERIVATIVES') fail('binary guard state mismatch')
...
if (binary.docxPolicy !== 'ALLOW_ONLY_TEXT_EXTRACTABLE_WITHOUT_EMBEDDED_MEDIA_AFTER_D0_D1_GUARD') fail('DOCX media must remain denied')
...
if (contract.result?.docxEmbeddedMediaVisualAdmission !== 'DENIED_PENDING_LOCAL_PREFLIGHT') fail('DOCX media must stay denied')
...
if (!contract.residuals?.some((item) => item.id === 'ANON_LONG_PDF_AND_DOCX_MEDIA_VISUAL_PREFLIGHT' && item.state === 'NOT_SATISFIED')) fail('narrow residual missing')
...
console.log('P7 anonymization input guard PASS: D0-D1 pre-storage guards active; images and visual PDFs up to 5 pages use local reviewed PNG derivatives; long visual PDFs and DOCX media remain denied; Tier 2 NOT_ADMITTED')
