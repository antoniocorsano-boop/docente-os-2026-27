...
const docxSemanticTests = fs.readFileSync('product/src/core/privacy/local-docx-media-semantic-preflight.test.ts', 'utf8')
...
for (const token of ['inspectDocxForLocalSemanticDerivative', 'auditDocxPackageMedia', 'inspectRawZipCentralDirectory', "'[Content_Types].xml'", 'parseOpcContentTypes', 'resolveOpcContentType', 'allPackageMediaSurfaced', 'mammoth.images.imgElement', 'readAsArrayBuffer', 'MAX_LOCAL_DOCX_MEDIA_ITEMS', 'MAX_LOCAL_DOCX_MEDIA_BYTES', 'MAX_LOCAL_DOCX_MEDIA_ITEM_BYTES', "'image/png'", "'image/jpeg'", "'image/webp'", 'externalFileAccess: false', 'includeEmbeddedStyleMap: false', 'DOCX_REFERENCED_MEDIA_SEMANTIC_REVIEWABLE']) if (!docxSemantic.includes(token)) fail(`staged DOCX semantic-preflight token missing: ${token}`)
for (const token of ['nomi duplicati', 'Override OPC', 'manifest OPC dichiara un tipo vietato', 'firma reale']) if (!docxSemanticTests.includes(token)) fail(`DOCX ZIP/OPC regression test missing: ${token}`)
...
console.log('P7 anonymization input guard PASS: canonical schema 6 admission remains unchanged; staged DOCX semantic PNG workbench now requires raw ZIP central-directory integrity, OPC content-type coherence and package-wide media review coverage; long PDFs and unsupported/over-budget DOCX media remain denied; Tier 2 NOT_ADMITTED')
