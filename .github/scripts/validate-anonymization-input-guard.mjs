...
for (const token of ['inspectDocxForLocalSemanticDerivative', 'auditDocxPackageMedia', 'inspectRawZipCentralDirectory', "'[Content_Types].xml'", 'parseOpcContentTypes', 'resolveOpcContentType', 'allPackageMediaSurfaced', 'mammoth.images.imgElement', 'readAsArrayBuffer', 'MAX_LOCAL_DOCX_MEDIA_ITEMS', 'MAX_LOCAL_DOCX_MEDIA_BYTES', 'MAX_LOCAL_DOCX_MEDIA_ITEM_BYTES', "'image/png'", "'image/jpeg'", "'image/webp'", 'externalFileAccess: false', 'includeEmbeddedStyleMap: false', 'DOCX_REFERENCED_MEDIA_SEMANTIC_REVIEWABLE']) if (!docxSemantic.includes(token)) fail(`DOCX semantic-preflight token missing: ${token}`)
