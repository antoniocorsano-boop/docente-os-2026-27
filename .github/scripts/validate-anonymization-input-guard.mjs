...
if (contract.result?.scannedDocumentVisualAdmission !== 'PARTIAL_SINGLE_PAGE_ONLY') fail('scanned-document admission must remain partial until governance promotion')
if (contract.result?.multiPagePdfVisualAdmission !== 'DENIED_PENDING_LOCAL_PREFLIGHT' || contract.result?.docxEmbeddedMediaVisualAdmission !== 'DENIED_PENDING_LOCAL_PREFLIGHT') fail('remaining visual document classes must stay denied in canonical governance')
...
for (const token of ['SINGLE_PAGE_VISUAL_REVIEWABLE', 'MULTI_PAGE_VISUAL_REVIEWABLE', 'MULTI_PAGE_VISUAL_BLOCKED', 'MAX_LOCAL_VISUAL_PDF_PAGES', 'classifyLocalPdfForVisualPreflight']) if (!pdfClassifier.includes(token)) fail(`local PDF classifier token missing: ${token}`)
...
for (const token of ['getDocumentProxy', 'MAX_LOCAL_VISUAL_PDF_PAGES', 'page.render', 'fillRect', 'canvas.toBlob', "'scansione-anonima.png'", 'PDF resta nel browser']) if (!pdfWorkbench.includes(token)) fail(`bounded PDF workbench token missing: ${token}`)
if (!pdfWorkbench.includes('count > MAX_LOCAL_VISUAL_PDF_PAGES')) fail('bounded PDF workbench must fail closed beyond the page cap')

console.log('P7 anonymization input guard PASS: canonical governance remains single-page; bounded multi-page local workbench is staged with explicit page cap and fail-closed overflow; Tier 2 NOT_ADMITTED')
