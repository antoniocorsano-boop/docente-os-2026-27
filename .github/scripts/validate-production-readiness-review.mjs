...
if (review.schemaVersion !== 5) fail('schemaVersion must be 5')
...
if (review.productionActivationDecision !== 'HOLD') fail('production activation must remain HOLD until persistent off-site destination is configured and a human release decision is made')
...
const requiredSatisfied = [
  ...
  'SUPABASE_AUTH_SERVICE_RECOVERY','OFFSITE_STORAGE_RECOVERY_REHEARSAL',
...
if (storageRecovery.residual?.id !== 'OFFSITE_STORAGE_PERSISTENT_DESTINATION' || storageRecovery.residual?.state !== 'NOT_CONFIGURED' || storageRecovery.residual?.classification !== 'ACTIVATION_BLOCKER') fail('persistent off-site destination residual mismatch')
...
if (blockers.get('OFFSITE_STORAGE_PERSISTENT_DESTINATION')?.classification !== 'BLOCKER' || blockers.get('OFFSITE_STORAGE_PERSISTENT_DESTINATION')?.state !== 'NOT_CONFIGURED') fail('persistent off-site destination must be the sole activation blocker')
for (const id of ['OFFSITE_STORAGE_RECOVERY','SUPABASE_AUTH_SERVICE_RECOVERY','INCIDENT_ESCALATION_MINIMUM','DB_LOGICAL_RESTORE','RESTORE_REHEARSAL']) {
...
if (review.nextGate?.id !== 'P7-OFFSITE-STORAGE-DESTINATION') fail('next gate must be persistent off-site Storage destination')
if (review.nextGate?.mayCreateInactiveProduction !== false || review.nextGate?.mayCreateActiveProduction !== false || review.nextGate?.mayAcceptRealUserData !== false) fail('persistent destination gate cannot activate Production or accept real data')

console.log(`Production readiness PASS: activation=${review.productionActivationDecision}, storageRehearsal=${storageRecovery.result}, residual=${storageRecovery.residual.id}, blockers=${review.activationBlockers.length}`)
