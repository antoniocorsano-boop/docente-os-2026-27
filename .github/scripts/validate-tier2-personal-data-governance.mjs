...
if (governance.schemaVersion !== 3 || governance.gate !== 'P7-TIER2-PERSONAL-DATA-GOVERNANCE') fail('gate/schema mismatch')
if (governance.state !== 'T2A_T2B_COMPLETE_BLOCKED') fail('governance must remain blocked after T2A/T2B')
...
for (const [id, evidence] of required) {
  ...
  const expectedState = ['T2A_DATA_MINIMIZATION_POLICY','T2B_APPLICATION_RETENTION_DELETION'].includes(id) ? 'SATISFIED' : 'NOT_SATISFIED'
...
console.log('P7 Tier 2 governance PASS: T2A/T2B satisfied; T2C/T2D outstanding; Tier 2 remains NOT_ADMITTED')
