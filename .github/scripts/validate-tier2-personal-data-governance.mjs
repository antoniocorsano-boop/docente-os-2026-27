...
if (governance.schemaVersion !== 5 || governance.gate !== 'P7-TIER2-PERSONAL-DATA-GOVERNANCE') fail('gate/schema mismatch')
...
if (t2d?.evidence !== 'ops/tier2-dedicated-privacy-review.json' || t2d?.reviewEvidenceComplete !== true || t2d?.blockingResidualCount !== 5) fail('T2D governance linkage incomplete')
...
if (privacyReview.residualRiskReview?.assessment !== 'BLOCKED' || privacyReview.residualRiskReview?.blockingResiduals?.length !== 5) fail('T2D blocker set mismatch')
...
console.log('P7 Tier 2 governance PASS: T2A/T2B/T2C satisfied; T2D review complete but externally blocked; Tier 2 remains NOT_ADMITTED')
