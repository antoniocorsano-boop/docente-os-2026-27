...
if (governance.schemaVersion !== 1 || governance.gate !== 'P7-TIER2-PERSONAL-DATA-GOVERNANCE') fail('gate/schema mismatch')
if (governance.state !== 'FOUNDATION_READY_BLOCKED') fail('foundation must remain blocked before workstream closure')
...
for (const [id, evidence] of required) {
  ...
  if (item.state !== 'NOT_SATISFIED') fail(`${id} must remain NOT_SATISFIED in foundation state`)
...
if (governance.nextAuthorizedWorkstream !== 'T2A_DATA_MINIMIZATION_POLICY') fail('first workstream must be data minimization')

console.log('P7 Tier 2 personal-data governance foundation PASS: 4 blocking workstreams; Tier 2 remains NOT_ADMITTED')
