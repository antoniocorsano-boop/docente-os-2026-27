...
7. L'ultimo fattore TOTP verificato non può essere rimosso: il vincolo è applicato lato server immediatamente prima dell'unenroll, non soltanto nella UI.
...
L'enrollment può usare l'SDK Auth dal client nella superficie già protetta AAL2. La rimozione di un fattore **verificato** passa invece da una server action che rivalida la sessione, richiede AAL2, rilegge i fattori verificati e autorizza l'unenroll solo se il fattore richiesto è realmente verificato e dopo la rimozione ne resta almeno uno.

### Sessioni

- revoca delle altre sessioni usa il contratto provider senza invalidare quella corrente;
- la revoca impedisce il rinnovo delle altre sessioni, mentre access token già emessi possono restare validi fino alla loro scadenza;
...
Il gestore MFA distingue le operazioni di enrollment dalle operazioni che riducono il numero di fattori verificati.
...
- la rimozione di un fattore verificato deve passare da un boundary server-authoritative;
- il server deve rivalidare identità, AAL2, appartenenza del fattore e conteggio aggiornato dei fattori verificati prima dell'unenroll;
- l'ultimo fattore verificato deve essere rifiutato anche se il client tenta direttamente l'operazione;
- il controllo client “non rimuovere l'ultimo fattore” resta soltanto una protezione UX;
...
- test negativo server-side/policy per rimozione dell'ultimo fattore e fattore non verificato;
