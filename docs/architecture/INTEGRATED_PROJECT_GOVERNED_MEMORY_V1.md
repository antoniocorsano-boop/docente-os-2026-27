...
- a provisional → approved authority transition, including a same-`curriculumVersionRef` transition, may be persisted in Docente OS only after **both** a server-verifiable Arena authority signal **and completed teacher revalidation**, with both bound to the **exact imported curricular context**: structural footprint, curriculum/curriculum-version identity, school year, discipline, grade plus section/cohort scope, applicability status and transition-rule identity, as well as the relevant institutional decision; the footprint remains a change detector rather than an authenticity proof, while authenticity comes from the server-verifiable Arena signal; persistence of the approved context occurs only after those two conditions are satisfied for that exact context;
