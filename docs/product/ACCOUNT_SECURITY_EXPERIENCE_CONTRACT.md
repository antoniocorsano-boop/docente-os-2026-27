...
Se lo stato MFA non può essere letto in modo affidabile, la UI deve mostrare **stato non disponibile** e non deve dedurre erroneamente che non esistano fattori verificati.
...
- l'ultimo fattore TOTP verificato non può essere rimosso;
- la UI disabilita l'azione quando vede un solo fattore, ma il vincolo deve essere riconfermato lato server immediatamente prima della rimozione;
- la rimozione di un fattore verificato richiede sessione valida AAL2 e verifica server-side del fattore e del conteggio aggiornato;
...
- **Revoca le altre sessioni** → mantiene attiva la sessione corrente e impedisce il rinnovo delle altre sessioni;
- **Esci da questa sessione** → termina la sessione corrente.

Le due azioni non devono essere fuse in un unico controllo ambiguo. La UI deve inoltre evitare di suggerire una revoca istantanea degli access token già emessi: possono restare validi fino alla loro scadenza.
...
- stato MFA temporaneamente non disponibile;
- sessione AAL2;
- operazione completata con esito positivo;
- errore di revoca sessioni;
- errore di gestione MFA;
- tentativo di rimuovere l'ultimo fattore verificato → rifiuto esplicito;
...
- affidare al solo client il vincolo di conservazione dell'ultimo fattore verificato;
...
Stato della slice al 13 settembre 2026:
