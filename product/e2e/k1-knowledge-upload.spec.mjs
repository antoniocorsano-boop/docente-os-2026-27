...
test('K1 Knowledge: stato comprensibile, errore recuperabile e retry reale', async ({ page }) => {
  await login(page)
  await page.goto('/knowledge')

  const upload = page.locator('input[type="file"][name="file"]')
  await upload.setInputFiles({
    name: 'k1-upload-recovery.txt',
    mimeType: 'text/plain',
  ...
  await expect(page.getByText('k1-upload-recovery.txt')).toBeVisible()
