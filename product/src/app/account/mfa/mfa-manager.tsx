...
import { canRemoveVerifiedMfaFactor, isVerifiedMfaFactor } from '@/core/security/account-security-policy'
import { createClient } from '@/lib/supabase/client'
import { removeVerifiedMfaFactor } from './actions'
...
export function MfaManager({ initialFactors }: MfaManagerProps) {
  ...
  async function removeFactor(factorId: string) {
    ...
    setBusy(true)
    setMessage(null)
    const result = await removeVerifiedMfaFactor(factorId)
    setBusy(false)

    if (!result.ok) {
      if (result.reason === 'last_factor') {
        setMessage('Per mantenere obbligatoria la protezione MFA non puoi rimuovere l’ultimo autenticatore verificato.')
        await refreshFactors()
        return
      }
      if (result.reason === 'mfa_required') {
        setMessage('La sessione non è più verificata ad AAL2. Verifica di nuovo il secondo fattore e riprova.')
        return
      }
      if (result.reason === 'session_required') {
        setMessage('La sessione non è più valida. Accedi di nuovo prima di gestire gli autenticatori.')
        return
      }
      if (result.reason === 'factor_not_verified') {
        setMessage('L’autenticatore selezionato non risulta più tra i fattori verificati.')
        await refreshFactors()
        return
      }
