...
import './knowledge-calendar-integration.test'
import {
...
test('rifiuta giorni esterni o date locali non valide', () => {
  assert.throws(() => assertCalendarDayWithinAcademicYear('2026-08-31', academicYear), /active academic year/)
  assert.throws(() => assertCalendarDayWithinAcademicYear('2027-09-01', academicYear), /active academic year/)
  assert.throws(() => assertCalendarDayWithinAcademicYear('2026-02-30', academicYear), /valid local date/)
})

test('accetta un evento intera giornata anche su più giorni entro l anno scolastico', () => {
  assert.doesNotThrow(() => assertCalendarEventWithinAcademicYear({
    startsOn: '2026-09-03',
    endsOn: '2026-09-05',
    allDay: true,
    startTime: null,
...
test('rifiuta eventi con ordine date errato o fuori anno scolastico', () => {
  assert.throws(() => assertCalendarEventWithinAcademicYear({
    startsOn: '2026-09-05',
    endsOn: '2026-09-03',
    allDay: true,
    startTime: null,
    endTime: null,
  }, academicYear), /end date must not precede/)
...
test('accetta solo eventi orari nello stesso giorno con orari validi e crescenti', () => {
  ...
  assert.throws(() => assertCalendarEventWithinAcademicYear({
    ...
    startTime: '12:00',
    endTime: '09:00',
  }, academicYear), /end time must be later/)
