...
test('la fonte e l’evento restano nello stesso anno scolastico e i riferimenti residui non bloccano la ricreazione', () => {
  ...
  assert.match(actionSource, /knowledge\.replaceTargetRef/)
  ...
  assert.match(integrityMigrationSource, /security definer/)
  assert.match(integrityMigrationSource, /revoke all on function private\.cleanup_calendar_event_knowledge_source\(\) from authenticated/)
  assert.match(integrityMigrationSource, /validation_status = 'AUTO'/)
  assert.match(integrityMigrationSource, /knowledge asset academic year is immutable while a calendar event is linked/)
  assert.match(integrityMigrationSource, /ce\.source_knowledge_unit_id = ku\.id/)
})
