...
const section: AnnualPlanSection = {
  id: 'section-3c', workspaceId: 'workspace-1', academicYearId: 'year-1', grade: 'TERZA', sectionCode: 'C',
...
function asset(overrides: Partial<KnowledgeAsset> = {}): KnowledgeAsset {
  return {
    id: 'asset-3c', workspaceId: 'workspace-1', academicYearId: 'year-1', assetKind: 'WEB', sourceProvider: 'MANUAL',
    sourceLocator: 'https://www.canva.com/d/example', originalName: '3C - Leggo la tecnologia come sistema', originalText: null,
    mimeType: null, byteSize: null, sha256: null, processingStatus: 'INDEXED',
    sourceMetadata: {
      docenteOsResource: 'CLASS_LESSON_MATERIAL', provider: 'CANVA', resourceKind: 'PRESENTATION', audience: 'STUDENT',
      approvalState: 'PENDING_HUMAN', targetDate: '2026-09-11', sectionId: 'section-3c', canonicalBinding: 'PRE_CANONICAL_DIAGNOSTIC',
      classroomSteps: [
    ...
    currentGenerationId: null, contentCategory: 'TEACHING_RESOURCE', disciplines: ['Tecnologia'], classLabels: ['3C'],
    contextStatus: 'REVIEWED', reliability: 'VERIFIED', capturedAt: '2026-09-10T10:00:00Z', createdBy: 'user-1',
...
test('builds an in-class session only for the exact class and a safe external source', () => {
  ...
  assert.equal(view.textGenerationAvailable, false)
...
test('generative capabilities become visible only when the server explicitly enables them', () => {
  const view = buildClassroomSessionView(section, asset(), {
    textGenerationAvailable: true,
    imageGenerationAvailable: true,
  })
  assert.ok(view)
  assert.equal(view.textGenerationAvailable, true)
  assert.equal(view.imageGenerationAvailable, true)
})

test('rejects a material explicitly bound to another canonical section', () => {
  const mismatched = asset({ sourceMetadata: { ...asset().sourceMetadata, sectionId: 'section-3a' } })
  assert.equal(buildClassroomSessionView(section, mismatched), null)
})

test('quick support is grounded in prepared lesson hints and remains deterministic fallback', () => {
  const view = buildClassroomSessionView(section, asset())
  assert.ok(view)
