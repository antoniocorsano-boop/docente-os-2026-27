  ...
  return (
    <div className="appShell">
      ...
      <main className="workSurface classesSurface">
        ...
        <section className="plannerHeader"><div><p className="contextLine">{context.workspace.name} · {context.academicYear?.label ?? 'Anno scolastico'}</p><h1>Classi</h1><p className="dayLine">Ritrova documenti, materiali e attività attraverso le classi e le sezioni a cui li hai collegati.</p></div></section>
        {classes.length ? <div className="classesGrid">{classes.map(([label, count]) => <Link className="classCard" href={`/knowledge?classLabel=${encodeURIComponent(label)}`} key={label}><span>CLASSE</span><h2>{label}</h2><p>{count} {count === 1 ? 'contenuto collegato' : 'contenuti collegati'}</p><strong>Apri i contenuti <i aria-hidden>→</i></strong></Link>)}</div> : <section className="emptyClasses"><span>NESSUNA CLASSE ANCORA COLLEGATA</span><h2>Le classi compariranno qui mentre organizzi i tuoi contenuti.</h2><p>In Conoscenza puoi indicare classe e sezione di ogni documento o materiale. DOCENTE OS costruirà automaticamente questa vista senza duplicare le informazioni.</p><Link href="/knowledge">Collega i contenuti alle classi</Link></section>}
      </main>
