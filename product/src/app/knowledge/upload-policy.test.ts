...
test('sanitizes filenames before building workspace-and-user-scoped storage paths', () => {
  ...
  assert.equal(
    buildKnowledgeObjectPath('workspace-1', 'user-1', 'verbale scuola.pdf', 'object-1'),
    'workspace-1/user-1/object-1-verbale_scuola.pdf',
  )
})
...
test('accepts the documented 20 MB boundary and rejects larger files', () => {
  const base = {
    ...
    ownerUserId: 'user-1',
    objectPath: 'workspace-1/user-1/object-1-file.pdf',
...
test('fails closed when the uploaded object is not scoped to the current workspace', () => {
  assert.deepEqual(validateKnowledgeUploadReference({
    workspaceId: 'workspace-1',
    ownerUserId: 'user-1',
    objectPath: 'workspace-2/user-1/object-1-file.pdf',
    originalName: 'file.pdf',
    mimeType: 'application/pdf',
    byteSize: 100,
...
test('fails closed when the uploaded object belongs to another user in the same workspace', () => {
  assert.deepEqual(validateKnowledgeUploadReference({
    workspaceId: 'workspace-1',
    ownerUserId: 'user-1',
    objectPath: 'workspace-1/user-2/object-1-file.pdf',
    originalName: 'file.pdf',
    mimeType: 'application/pdf',
    byteSize: 100,
  }), { valid: false, code: 'invalid_path' })
...
test('rejects unsupported mime types even when the path is otherwise valid', () => {
  assert.deepEqual(validateKnowledgeUploadReference({
    workspaceId: 'workspace-1',
    ownerUserId: 'user-1',
    objectPath: 'workspace-1/user-1/object-1-file.exe',
