...
test('builds a local deterministic proposed activation question draft', () => {
  ...
  assert.match(proposal.body, /elementi o relazioni/)
  assert.match(proposal.body, /ipotesi/)
  assert.doesNotMatch(proposal.body, /Che cosa sai già/)
  assert.match(proposal.cue ?? '', /Riconoscere proprietà e usi dei materiali/)
