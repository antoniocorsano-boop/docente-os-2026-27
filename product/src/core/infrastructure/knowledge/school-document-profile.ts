...
function inferClasses(text: string) {
  ...
  if (/classe prima|classi?\s+i\b|secondaria\s+[-—]\s+i\b/.test(text)) labels.add('Classe prima')
  if (/classe seconda|classi?\s+ii\b|secondaria\s+[-—]\s+ii\b/.test(text)) labels.add('Classe seconda')
  if (/classe terza|classi?\s+iii\b|secondaria\s+[-—]\s+iii\b/.test(text)) labels.add('Classe terza')
