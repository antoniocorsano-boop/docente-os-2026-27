...
type CurriculumRpcClient = {
  rpc: (
    name: 'annual_plan_curriculum_current' | 'persist_eco02_provisional_curriculum_adoption',
...
export class SupabaseAnnualPlanCurriculumRepository {
  async persist(input: {
    ...
    const { data, error } = await rpc.rpc('persist_eco02_provisional_curriculum_adoption', {
